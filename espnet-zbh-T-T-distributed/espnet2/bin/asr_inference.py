...
class Speech2Text:
    ...
    def __init__(
        ...
        self.dtype = dtype
        self.nbest = nbest
        sefl.beam_size = beam_size
        self.simulate_streaming = simulate_streaming
        self.attention_rescore = attention_rescore
    ...
    def __call__(
        ...
        if self.attention_rescore:
            nbest_hyps = self.beam_search.batchfy(nbest_hyps)
            nbest_scores = nbest_hyps.score
            nbest_hyps_yseq = nbest_hyps.yseq
            nbest_hyps_yseq = [list(filter(lambda x:x!=4233 and x!=0, y.tolist())) for y in nbest_hyps_yseq]
            from torch.nn.utils.rnn import pad_sequence
            hyps_pad = pad_sequence([torch.tensor(hyp, dtype=torch.long) for hyp in nbest_hyps_yseq], True, -1)
            from espnet.nets.pytorch_backend.transformer.add_sos_eos import add_sos_eos
            ys_in_pad, _ = add_sos_eos(hyps_pad, 4233, 4233, -1)
            hyps_lens = torch.tensor([len(i) for i in nbest_hyps_yseq], dtype=torch.long)
            hyps_lens = hyps_lens + 1
            encoder_out = enc.repeat(self.beam_size, 1, 1)
            encoder_len = enc_len.repeat(self.beam_size)
            decoder_out, _ = self.asr_model.decoder(encoder_out, encoder_len, ys_in_pad, hyps_lens)
            decoder_out = torch.nn.functional.log_softmax(decoder_out, dim=-1)
            best_score = -float('inf')
            best_index = 0
            newscores = []
            for i, hyp in enumerate(nbest_hyps_yseq):
                score = 0.0
                for j, w in enumerate(hyp):
                    score += decoder_out[i][j][w]
                score += decoder_out[i][len(hyp)][4233]
                score = score + nbest_scores[i]*0.5
                if score > best_score:
                    best_score = score
                    best_index = i
                newscores.append(score)
            nbest_hyps = nbest_hyps[best_index]
